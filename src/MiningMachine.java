...
*智能矿机
*/
public class MiningMachine extends Block {
  ...
  public class MiningMachineBuild extends Building {
    ...
    public void draw() {
      ...
      Lines.stroke((0.7f + Mathf.absin(20, 0.7f)), Color.valueOf("#8AA3F4"));
            
      for(int i = 0; i < resources.size; i++){
        float rot = rotation + i * 360f/resources.size - Time.time * 0.5f;
        Lines.swirl(x, y, orbRadius + 3f, 0.14f, rot);
      }
      Drawf.light(x, y, tilesize * 1.5f, Color.valueOf("#8AA3F4"), warmup);
    ...
    private void setup(Table table) {
      table.clear();
      for(int i = 0;i<resources.size;i++) {
        final int a = i;
        table.button("撤销第"+(a+1)+"个",() -> {
          resources.remove(a);
          setup(table);
        });
        table.row();
  ...
  private class Res {
    ...
    public void update(MiningMachineBuild entity) {
    ...
    public void draw(MiningMachineBuild entity) { 
      if(x == -1 || y == -1 ) return;
      ...
      Drawf.light(x, y, resRange * 1.5f, Color.valueOf("#8AA3F4"), curStroke * 0.8f);
